...
const formSchema = z.object({
  ...
  message: z.string().min(10, { message: 'I think you should write more than that if you want to send me an email' }).max(1000, { message: 'Message is too long' }),
});
...
const ContactForm = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast({
        ...
        variant: 'destructive',
        action:
          <ToastAction altText="Open mailing app" onClick={() => { window.open('mailto:contact@cosmin.zip', '_blank') }}>
            Send Manually
          </ToastAction>,
      });
    }
  ...
  return (
    <Form {...form}>
      <form
        onSubmit={form.handleSubmit(onSubmit)} className='flex md:flex-row flex-col gap-3'
      >
        <FormField
